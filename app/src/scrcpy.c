...
#endif
    struct sc_websocket_sink websocket_sink;
...
#endif
    bool websocket_sink_initialized = false;
...
#endif
    // WebSocket sink 不需要解码器，直接转发 packet
    // needs_video_decoder |= !!options->websocket_server;
...
aoa_complete:
    ...
    // WebSocket sink for remote viewing
    if (options->websocket_server) {
        ...
        if (!sc_websocket_sink_init(&s->websocket_sink, options->websocket_server, serial)) {
            goto end;
        }

        // 直接连接到 packet source，转发原始 H.264 packet
        struct sc_packet_source *src = &s->video_demuxer.packet_source;
        sc_packet_source_add_sink(src, &s->websocket_sink.packet_sink);
        
        // 如果 controller 已经初始化，设置到 websocket_sink
        if (controller) {
            sc_websocket_sink_set_controller(&s->websocket_sink, controller);
        }

        websocket_sink_initialized = true;
        LOGI("WebSocket sink enabled (packet mode), server: %s, device: %s", 
             options->websocket_server, serial);
    }
    ...
    if (websocket_sink_initialized) {
        sc_websocket_sink_destroy(&s->websocket_sink);
    }
