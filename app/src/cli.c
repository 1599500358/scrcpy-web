...
static const struct sc_option options[] = {
    ...
    {
        .longopt_id = OPT_WEBSOCKET_PORT,
        .longopt = "websocket-server",
        .argdesc = "host:port",
        .text = "Connect to WebSocket relay server for remote viewing.\n"
                "Example: --websocket-server=localhost:8080",
    },
};
...
parse_args_with_getopt(struct scrcpy_cli_args *args, int argc, char *argv[],
    ...
    while ((c = getopt_long(argc, argv, optstring, longopts, NULL)) != -1) {
        switch (c) {
            ...
            case OPT_WINDOW_HEIGHT:
                ...
                break;
            case OPT_WEBSOCKET_PORT:
                opts->websocket_server = optarg;
                break;
    ...
    if (opts->video && !opts->video_playback && !opts->record_filename
            && !v4l2 && !opts->websocket_server) {
        LOGI("No video playback, no recording, no V4L2 sink, no WebSocket: video disabled");
        opts->video = false;
    }
